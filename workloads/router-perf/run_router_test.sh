...
echo "[orchestration]" > /tmp/workloads/inventory; echo "${ORCHESTRATION_HOST:-localhost} ansible_user=${ORCHESTRATION_USER:-root} ansible_ssh_common_args='-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null'" >> /tmp/workloads/inventory
...
../../utils/touchstone-compare/run_compare.sh mb ${baseline_router_uuid} ${compare_router_uuid}

...
deploy_perf_profile() {
  ...
  if [[ "${baremetalCheck}" == '"BareMetal"' ]]; then
    ...
    worker_count=0
    workers=$(oc get node -o name --no-headers -l node-role.kubernetes.io/worker-lb!="",node-role.kubernetes.io/worker-spk!="",node-role.kubernetes.io/workload!="",node-role.kubernetes.io/infra!="",node-role.kubernetes.io/worker-oslat!="",node-role.kubernetes.io/worker-cyclic!="",node-role.kubernetes.io/worker= | head -${NODE_COUNT} | sed -e 's/^node\///')

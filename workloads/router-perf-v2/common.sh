...
gen_mb_mix_config(){
  log "Generating config for termination ${termination}-${mix_size} with ${clients} clients ${keepalive_requests} keep alive requests and path ${URL_PATH}"
  local first=true
  # If mix_size is small we take only 1/10 of the routes of each termination
  if [[ ${mix_size} == "small" ]]; then
    local route_count=$((NUMBER_OF_ROUTES / 10))
  else
    local route_count=$NUMBER_OF_ROUTES
  fi
  (echo "["
  for mix_termination in http edge passthrough reencrypt; do
    ...
    done <<< $(oc get route -n http-scale-${mix_termination} --no-headers | awk '{print $2}' | head -n ${route_count})
  done
